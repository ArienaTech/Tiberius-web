...
/* --- Greek key (meander) engraving for the hero knot: gold band with
   square-spiral motifs between double border lines. Used as both the
   color map (white base × gold tint, dark engraved lines) and the bump
   map (dark = recessed) so the pattern reads as engraved relief. --- */
function makeGreekTexture(){
  const W = 1024, H = 256, c = document.createElement("canvas");
  c.width = W; c.height = H;
  const x = c.getContext("2d");
  x.fillStyle = "#ffffff"; x.fillRect(0,0,W,H);
  x.strokeStyle = "#6d5619"; x.lineCap = "butt"; x.lineJoin = "miter";
  x.lineWidth = 8;
  [16, H-16].forEach(y=>{ x.beginPath(); x.moveTo(0,y); x.lineTo(W,y); x.stroke(); });
  x.lineWidth = 4;
  [40, H-40].forEach(y=>{ x.beginPath(); x.moveTo(0,y); x.lineTo(W,y); x.stroke(); });
  const CELLS = 8, cw = W/CELLS;
  const pts = [[0,4],[4,4],[4,0],[1,0],[1,3],[3,3],[3,1],[2,1],[2,2]]; // square spiral
  const s = 104, u = s/4, ox0 = (cw - s)/2, oy = (H - s)/2;
  x.lineWidth = u * 0.42;
  for (let i=0;i<CELLS;i++){
    const ox = i*cw + ox0;
    x.beginPath();
    pts.forEach(([px,py],j)=> j ? x.lineTo(ox+px*u, oy+py*u) : x.moveTo(ox+px*u, oy+py*u));
    x.stroke();
  }
  const t = new THREE.CanvasTexture(c);
  t.wrapS = t.wrapT = THREE.RepeatWrapping;
  t.repeat.set(6, 3); // ~knot-length/circumference ratio keeps motifs square
  t.anisotropy = renderer.capabilities.getMaxAnisotropy();
  t.colorSpace = THREE.SRGBColorSpace;
  return t;
}

/* Scene 1 — hero: engraved gold torus knot.
   Unlike the matcap-shaded sculptures, this is a PBR standard material,
   so it needs real lights plus a PMREM environment map to read as metal. */
scene.add(new THREE.AmbientLight(0xffffff, 0.3));
const keyLight = new THREE.DirectionalLight(0xffffff, 1.5);
keyLight.position.set(5, 5, 5);
scene.add(keyLight);
const pmrem = new THREE.PMREMGenerator(renderer);
const heroEnvMap = pmrem.fromScene(new RoomEnvironment(), 0.04).texture;
pmrem.dispose();

/* float wrapper — gentle idle bob/wobble applied per-frame (drei <Float>
   equivalent: speed 1.4, rotationIntensity 0.4, floatIntensity 1.1) */
const knotFloat = new THREE.Group();
knotFloat.position.set(2.3, 0.35, 0);
knotFloat.scale.setScalar(1.45); // keep the on-screen footprint of the old 1.5-radius knot
scene.add(knotFloat);
const greekTex = makeGreekTexture();
const heroKnot = new THREE.Mesh(
  new THREE.TorusKnotGeometry(1, 0.32, isMobile ? 160 : 220, isMobile ? 24 : 36),
  new THREE.MeshStandardMaterial({
    color: "#e8b34a", metalness: 1, roughness: 0.18,
    map: greekTex, bumpMap: greekTex, bumpScale: 0.02,
    envMap: heroEnvMap, envMapIntensity: 1.7
  })
);
heroKnot.userData.spin = new THREE.Vector3(0.12, 0.16, 0);
knotFloat.add(heroKnot);
sculptures.push(heroKnot);
...
addGlow(new THREE.Vector3(2.3, 0.4, -1.2), 9, glowBrass, 0.5);
...
const rail = [
  ...
  { p:0.13, pos:V3(-1.6, 0.5,  4.2), look:V3( 2.3, 0.3,  0.0) },   // manifesto — slow push past knot
...
updateGL = function(progress, dt){
  ...
  // hero knot idle float (drei <Float> speed 1.4 / rotationIntensity 0.4 / floatIntensity 1.1)
  const ft = t * 1.4;
  knotFloat.rotation.x = Math.cos(ft / 4) * 0.4 / 8;
  knotFloat.rotation.y = Math.sin(ft / 4) * 0.4 / 8;
  knotFloat.rotation.z = Math.sin(ft / 4) * 0.4 / 20;
  knotFloat.position.y = 0.35 + Math.sin(ft / 2) * 0.11;
...
let rafVel = 0, lastCurrent = 0, hidden = document.hidden;
...
function raf(){
  ...
  // safety net for the one-shot reveals (see sweepReveals below)
  sweepReveals(now);
}
/* the loop is started at the bottom of initSite, once every binding
   below (reveals, sweeps) is in place */
...
/* ============================================================
   ...
   All hide-then-reveal effects used to be ScrollTriggers whose start
   positions were measured once at boot. Lazy-loaded images, font swaps
   and the FAQ accordion change the page height after that measurement,
   so a trigger's stored position could drift past its element — and
   since the CSS keeps this content at opacity:0 until its reveal runs,
   the element simply never appeared. IntersectionObserver reads the
   element's real on-screen position every frame (including the
   smooth-scroll transform), so reveals can't be orphaned by stale
   measurements; a slow rect sweep in the RAF loop backs it up.
   ============================================================ */
...
const revealIO = ("IntersectionObserver" in window)
  ? new IntersectionObserver(entries=>{
      entries.forEach(e=>{ if (e.isIntersecting) fireReveal(e.target); });
    }, { rootMargin: "0px 0px -10% 0px", threshold: 0 })
  : null;
function onEnterOnce(el, play){
  el.__reveal = play;
  pendingReveals.add(el);
  if (revealIO) revealIO.observe(el);
}
let sweepAt = 0;
function sweepReveals(now){
  if (!pendingReveals.size || now < sweepAt) return;
  sweepAt = now + 250;
  // at the bottom of the page the reveal line moves to the viewport edge,
  // otherwise elements living in the last 10% of the screen (contact meta,
  // footer details) could never cross it and would stay invisible
  const maxScroll = Math.max(1, (SMOOTH ? content.scrollHeight : document.documentElement.scrollHeight) - innerHeight);
  const line = smooth.current >= maxScroll - 4 ? innerHeight : innerHeight * 0.92;
  pendingReveals.forEach(el=>{
    // fire anything at or above the reveal line — including elements the
    // observer missed because a jump skipped them past the viewport
    if (el.getBoundingClientRect().top < line) fireReveal(el);
  });
}
...
document.querySelectorAll("[data-split]").forEach(h=>{
  ...
  onEnterOnce(h, ()=>gsap.to(words, { yPercent:0, duration:1.25, ease:"power4.out", stagger:0.035 }));
});
...
(function(){
  ...
  onEnterOnce(copy, ()=>gsap.to(copy.querySelectorAll(".word"), {
...
gsap.utils.toArray("[data-reveal]").forEach(el=>{
  ...
  onEnterOnce(el, ()=>gsap.to(el, { opacity:1, y:0, duration:1.1, ease:"power3.out" }));
});
...
document.querySelectorAll(".stat-num").forEach(el=>{
  ...
  const obj = { v:0 };
  onEnterOnce(el, ()=>gsap.to(obj,{ v:end, duration:1.8, ease:"power2.out",
...
(function(){
  ...
  if (!slides.length || !counter || !bar || !nextBtn || !prevBtn) return;
  let idx = 0, timer;
  const AUTO = 6.5;

  function go(n, dir){
    const target = (n + slides.length) % slides.length;
    if (target === idx) { restartBar(); return; }
    const prev = slides[idx];
    idx = target;
    const next = slides[idx];
    /* rapid clicks (or auto-advance timers piling up in a background tab)
       used to stack conflicting tweens whose stale onComplete could strip
       .is-active off the slide that was actually showing, leaving the
       whole block empty — always converge on exactly one visible slide */
    gsap.killTweensOf(slides);
    slides.forEach(s=>{
      if (s !== prev && s !== next){ s.classList.remove("is-active"); gsap.set(s, { opacity:0 }); }
    });
    gsap.to(prev, { opacity:0, y:-26, duration:0.5, ease:"power2.in",
      onComplete:()=>{ if (slides[idx] !== prev) prev.classList.remove("is-active"); } });
    next.classList.add("is-active");
    gsap.fromTo(next, { opacity:0, y: 34*dir }, { opacity:1, y:0, duration:0.85, ease:"power3.out", delay:0.28, overwrite:"auto" });
  ...
  function restartBar(){
    ...
    clearTimeout(timer);
    if (!reduced) timer = setTimeout(()=>{
      // hold the carousel while the tab is hidden instead of advancing blind
      if (document.hidden) restartBar();
      else go(idx+1, 1);
    }, AUTO*1000);
  }
  nextBtn.addEventListener("click", ()=>go(idx+1, 1));
  prevBtn.addEventListener("click", ()=>go(idx-1, -1));
  if (voices) onEnterOnce(voices, restartBar);
  else restartBar();
})();
...
/* the page keeps changing height after boot (lazy images, font swaps,
   the FAQ accordion) — remeasure whenever it does, so the scroll range
   and the remaining scrubbed triggers never go stale */
let roTimer = 0;
const contentRO = ("ResizeObserver" in window)
  ? new ResizeObserver(()=>{ clearTimeout(roTimer); roTimer = setTimeout(finalRefresh, 150); })
  : null;
if (contentRO) contentRO.observe(content);

/* start the render loop last — every binding it services now exists */
raf();
