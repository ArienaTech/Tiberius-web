...
/* liquid champagne-gold — single bright key-light highlight, deep bronze falloff */
const matGold   = makeMatcap([[0,"#fffdf5"],[0.12,"#ffe9a8"],[0.32,"#e8bd6a"],[0.55,"#a97f34"],[0.8,"#3a2a10"],[1,"#0a0700"]]);

/* --- engraved Greek-key (meander) band, tiled around the hero knot's tube --- */
function makeGreekKeyPattern(){
  const period = 64, h = 128, cols = 8, w = period * cols;
  const c = document.createElement("canvas"); c.width = w; c.height = h;
  const x = c.getContext("2d");
  x.fillStyle = "#3d3d3d"; x.fillRect(0, 0, w, h);
  x.strokeStyle = "#ffffff"; x.lineWidth = h * 0.2;
  x.lineJoin = "miter"; x.lineCap = "square";
  const top = h*0.22, bot = h*0.78, midHi = h*0.36, midLo = h*0.64;
  x.beginPath();
  for (let i = 0; i < cols; i++){
    const bx = i * period;
    x.moveTo(bx, bot);
    x.lineTo(bx, top);
    x.lineTo(bx + period*0.5, top);
    x.lineTo(bx + period*0.5, midHi);
    x.lineTo(bx + period*0.25, midHi);
    x.lineTo(bx + period*0.25, midLo);
    x.lineTo(bx + period*0.75, midLo);
    x.lineTo(bx + period*0.75, top);
...
const greekKeyTex = makeGreekKeyPattern();
...
/* --- variant of sculptMaterial with an engraved surface pattern, used only
   for the hero knot; kept separate so every other shape's shader is
   byte-for-byte unchanged --- */
function sculptMaterialEngraved(matcap, patternTex, opts){
  return new THREE.ShaderMaterial({
    uniforms:{
      uMatcap:{ value: matcap },
      uPattern:{ value: patternTex },
      uPatternRepeat:{ value: new THREE.Vector2(...(opts.patternRepeat || [28, 5])) },
      uPatternDepth:{ value: opts.patternDepth !== undefined ? opts.patternDepth : 2.4 },
      uTime:{ value: 0 },
      uAmp:{ value: opts.amp },
      uFreq:{ value: opts.freq },
      uFresnelColor:{ value: new THREE.Color(opts.rim) },
    ...
    vertexShader: NOISE + `
      uniform float uTime, uAmp, uFreq;
      varying vec3 vN; varying vec3 vE; varying float vDist; varying vec2 vUv;
      void main(){
        vec3 p = position;
        float n = snoise(normal*uFreq + uTime*0.25);
        p += normal * n * uAmp;
    ...
    fragmentShader:`
      uniform sampler2D uMatcap, uPattern;
      ...
      void main(){
        vec3 n = normalize(vN);
        vec2 puv = vUv * uPatternRepeat;
        float hC = texture2D(uPattern, puv).r;
        float hX = texture2D(uPattern, puv + vec2(0.0025, 0.0)).r;
        float hY = texture2D(uPattern, puv + vec2(0.0, 0.0025)).r;
        vec3 bump = vec3(hC - hX, hC - hY, 0.0) * uPatternDepth;
        vec3 nB = normalize(n + bump);
        vec2 muv = nB.xy * 0.495 + 0.5;
        vec3 base = texture2D(uMatcap, muv).rgb;
        base *= mix(0.6, 1.4, hC);
        float fr = pow(1.0 - abs(dot(n, -normalize(vE))), uFresnelPow);
        vec3 col = base + uFresnelColor * fr * 1.35;
...
/* Scene 1 — hero: liquid gold torus knot with an engraved Greek-key band */
addSculpture(
  new THREE.TorusKnotGeometry(1.5, 0.44, isMobile ? 160 : 260, isMobile ? 24 : 40),
  sculptMaterialEngraved(matGold, greekKeyTex, { amp:0.05, freq:1.6, rim:"#ffdca0", rimPow:3.2 }),
...
addGlow(new THREE.Vector3(2.3, 0.4, -1.2), 9, glowBrass, 0.55);
