...
function makeGreekKeyPattern(){
  const period = 96, h = 160, cols = 8, w = period * cols;
  const c = document.createElement("canvas"); c.width = w; c.height = h;
  const x = c.getContext("2d");
  x.fillStyle = "#2a2a2a"; x.fillRect(0, 0, w, h);
  x.strokeStyle = "#ffffff"; x.lineJoin = "miter"; x.lineCap = "square";
  // top/bottom frame lines
  x.lineWidth = h * 0.05;
  x.beginPath();
  x.moveTo(0, h*0.10); x.lineTo(w, h*0.10);
  x.moveTo(0, h*0.90); x.lineTo(w, h*0.90);
  x.stroke();
  // repeating meander (greek key) motif
  x.lineWidth = h * 0.11;
  const top = h*0.24, bot = h*0.76, mid1 = h*0.40, mid2 = h*0.60;
  x.beginPath();
  for (let i = 0; i < cols; i++){
    ...
    x.lineTo(bx + period*0.30, mid1);
    x.lineTo(bx + period*0.30, mid2);
...
function sculptMaterialEngraved(matcap, patternTex, opts){
  return new THREE.ShaderMaterial({
    uniforms:{
      ...
      uPatternRepeat:{ value: new THREE.Vector2(...(opts.patternRepeat || [5, 1])) },
